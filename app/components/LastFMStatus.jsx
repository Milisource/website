...
export default function LastFMStatus() {
  ...
  useEffect(() => {
    ...
    async function fetchLastFMData() {
      ...
      // Prevent fetching if we've already fetched in the last 2 minutes
      const now = Date.now()
      if (now - lastFetchTime < 120000) return
    ...
    function handleVisibility() {
      ...
      // Did we fetch in the last 2 minutes and is the page visible? If all are true, fetch.
      if (isActive && Date.now() - lastFetchTime > 120000) {
        fetchLastFMData()
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12 animate-fade-in-up">
        ...
        <span className="ml-3 text-gray-600 dark:text-gray-300">Loading music...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      {/* Current Track */}
      {currentTrack && (
        <div className={`animate-on-load animate-fade-in-up ${hasLoaded ? 'animate-delay-100' : ''}`}>
      ...
      {recentTracks.length > 0 && (
        <div className={`animate-on-load animate-fade-in-up ${hasLoaded ? 'animate-delay-400' : ''}`}>
